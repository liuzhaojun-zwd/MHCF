...
class LinearProjectionAdapter(nn.Module):
    """线性投影Adapter - 最简单的替代方案"""
    
    def __init__(self, dim=768, modes=64):
        super().__init__()
        self.dim = dim
        self.modes = modes
        ...
        self.weight = nn.Parameter(torch.randn(dim, modes) * 0.02)
    ...
    def forward(self, x):
        ...
        # 线性投影
        x_proj = self.projection(patch_tokens.transpose(1, 2))  # (B, L, C)
        x_proj = x_proj.transpose(1, 2)  # (B, C, L)
        
        # 门控变换
        gate_weights = torch.sigmoid(self.gate(patch_tokens.transpose(1, 2))).transpose(1, 2)
        x_proj = x_proj * gate_weights
        
        # 应用权重
        x_proj_low = x_proj[:, :, :self.modes] * self.weight
        x_proj = torch.cat([x_proj_low, x_proj[:, :, self.modes:]], dim=-1)
        
        patch_tokens = x_proj.transpose(1, 2)
        return torch.cat([cls_token, patch_tokens], dim=1)

class ConvolutionAdapter(nn.Module):
    """卷积变换Adapter - 使用1D卷积模拟频域操作"""
    
    def __init__(self, dim=768, modes=64, kernel_size=3):
        super().__init__()
        self.dim = dim
        self.modes = modes
        ...
        # 权重融合
        self.weight = nn.Parameter(torch.randn(dim, modes) * 0.02)
        self.fusion_conv = nn.Conv1d(dim * 4, dim, 1)
    ...
    def forward(self, x):
        ...
        for conv in self.conv_layers:
            conv_out = conv(patch_tokens)
            conv_outputs.append(conv_out)
        
        # 融合多尺度特征
        x_conv = torch.cat(conv_outputs, dim=1)
        x_conv = self.fusion_conv(x_conv)
        
        # 应用权重变换（模拟频域滤波）
        x_conv_low = x_conv[:, :, :self.modes] * self.weight
        x_conv = torch.cat([x_conv_low, x_conv[:, :, self.modes:]], dim=-1)
        
        patch_tokens = x_conv.transpose(1, 2)
        return torch.cat([cls_token, patch_tokens], dim=1)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, modal_prompt = None, F=None):
